  ...
  if (TotalEAR < EYE_AR_THRESH) {
    ...
    if ((BLINKS_TEMP_COUNTER * LOG_EVERY_MS) >= EYE_CLOSE_EYES_CONSEC_MS) {
      ...
      if (actualTickTarget == 1) {
        ...
        alert("PRUEBA FALLIDA. Haz realizado le pestañeo a destiempo, vuelve a comenzar la prueba");
  ...
  if (Date.now() - LAST_MODEL_LOG > LOG_EVERY_MS) {
    ...
    if (Point10Inside &&
      ...
      if (VALIDATION_TICK_NUMBER * LOG_EVERY_MS >= VALIDATION_CONSEC_MS) {
        ...
        // Run Blink Test
        document.getElementById("Progress_Bar").value = Math.floor((LOG_EVERY_MS * (VALIDATION_TICK_NUMBER + BLINK_TEST_TICK_NUMBER)) / (LIVE_TEST_CONSEC_MS + VALIDATION_CONSEC_MS) * 100);
        BlinkTest(data);
        ...
        document.getElementById("Progress_Bar").value = Math.floor((LOG_EVERY_MS * VALIDATION_TICK_NUMBER) / (LIVE_TEST_CONSEC_MS + VALIDATION_CONSEC_MS) * 100);
        BLINK_TEST_INPROGRESS = false;
        BLINK_TEST_TICK_NUMBER = 0;
      }
    }
...
function TakeSelfie()
{
  const canvasPhotoPeople = document.getElementById("Photo_People");
  // scale the canvas accordingly
  canvasPhotoPeople.width = 400;
  canvasPhotoPeople.height = 300;
  // draw the video at that frame
  canvasPhotoPeople.getContext('2d')
    .drawImage(video, 0, 0, canvasPhotoPeople.width, canvasPhotoPeople.height);
...
async function renderPrediction() {
  ...
  const facepred = await FACEMESH_MODEL.estimateFaces({
    input: video
  });
  const cocoSSDpred = await COCOSSD_MODEL.detect(video); 
// COCO SDD LOGIC - FOR TESTING ONLY
  if(COCOSSD_FindBook(cocoSSDpred))
  {
    const canvasPhotoCard = document.getElementById("Photo_Card");
    // scale the canvas accordingly
    canvasPhotoCard.width = 400;
    canvasPhotoCard.height = 300;
    // draw the video at that frame
    canvasPhotoCard.getContext('2d')
      .drawImage(video, 0, 0, canvasPhotoCard.width, canvasPhotoCard.height);
    }
...
function COCOSSD_FindBook(Predictions)
{
  if(Predictions != null && Predictions.length > 0)
  {
    for (var counter = 0; counter < Predictions.length; counter++) {
  
      if (Predictions[counter].class == "book") {
        return true;
      }
...
async function main() {
  ...
  await tf.ready();
  FACEMESH_MODEL = await faceLandmarksDetection.load(
    faceLandmarksDetection.SupportedPackages.mediapipeFacemesh);
  
  COCOSSD_MODEL = await cocoSsd.load({
    base: 'mobilenet_v2'
  });
